...
* 注意：如果打不开控制台，可以在properties文件里改下控制台代理ip试一试，改成当前管理节点的
...
14.**冷迁移**:想要进行冷迁移需要确认主存储是本地存储，共享存储不能进行冷迁移，只能进行在线迁移。在本地存储想要进行冷迁移，需要先停止云主机，再将云主机迁移至其他计算节点。迁移的速度与两台主机的网络配置有关,如果网络配置较低,迁移速度可能较慢。具体步骤如下示
...
**注意:**
1. 如果云主机正在进行大量的I\/O并发,此时创建快照,有可能会有数据丢失的风险,请谨慎操作。以防万一,可以在云主机停止后,再创建快照。如果用户从低版本升级到新版本,在线快照并不适用于正在运行中的云主机,需要云主机重启后才可生效。
2. 如果要恢复快照要先停止云主机。
3. 创建的第一个快照作为一个Base，也就是说第一个删除了，后面的都会删除。
