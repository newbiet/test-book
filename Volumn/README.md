...
如果勾选VirtioSCSI,数据云盘具有全局SCSI wwn识别标记，ZStack内部自动创建唯一识别ID，并将此识别ID带到云盘设备。云主机（例如Linux）启动后，从/dev/disk/by-id/下可查看wwn。wwn标记方便管理员和用户挂载和卸载数据云盘。
...
在ceph存储的环境下添加云盘时选择ceph对应的主存储出现ceph pool选项，点击ceph pool在出现的下拉选项中出现在主存储中已创建好的pool；如未指定pool，则默认为添加到数据盘pri-v-d对应的pool内。

**注意：**只有在Ceph存储下创建云盘时才出现添加ceph pool选项；用ceph pool创建的云盘创建成功后直接会被实例化。
...
用户可以选择迁移云主机（需要首先停止云主机），也可以选择迁移云盘。在卸载云盘后，点击云盘详情的操作，再点击迁移，在弹出的对话框中选择希望迁移的目标物理机后，ZStack就可以将该云盘迁移到目标物理机。如图13-4所示：
