...
针对Sftp类型的镜像服务器，可支持添加多个镜像服务器。如图10-2-2所示。
...
![png](../images/10-2-2.png "图10-2-2  Sftp类型镜像服务器管理界面")
###### 图10-2-2  Sftp类型镜像服务器管理界面

针对Ceph或FusionStor类型的镜像服务器，支持在挂载栏添加同一集群的多个监控节点。以Ceph存储为例，如图10-2-3所示。而且也可以在挂载页面修改相应的SSH用户名、SSH密码、SSH端口号、Ceph Mon的端口号。

![png](../images/10-2-3.png "图10-2-3  Ceph类型镜像服务器管理界面")
###### 图10-2-3  Ceph类型镜像服务器管理界面

针对镜像市场类型的镜像服务器，在同一节点上不能与sftp类型并存。

![png](../images/10-2-4.png "图10-2-4  镜像仓库类型镜像服务器管理界面")
###### 图10-2-4  镜像仓库类型镜像服务器管理界面
