...
ZStack支持采用IPsec隧道技术实现站点到站点（site-to-site）的虚拟私有网络（VPN）连接。
...
为了更好的演示IPsec隧道功能，我们准备了**两个zstack环境**，其网络情况如下：

![png](../images/14-4-6-1.png "图14-4-6-1 通过IPsec连接两个ZStack网络")

###### 图14-4-6-1  通过IPsec连接两个ZStack网络
...
点击ZStack1菜单栏的网络中的IPsec隧道按钮，进入IPsec隧道界面，如图14-4-6-2所示。
...
公共网络：选择一个可用的公有网络1（参考[14.4.1](/Network/VR-network.md)），该网络与ZStack2的公有网络2必须可以通信。

公共网络IP：指定一个个公有网络段中的IP。

本地子网：选择一个可用的私有网络1（参考[14.4.1](/Network/VR-network.md)）。

远端地址：ZStack2中的公共网络IP。我们这里从公网2中选择一个172.20.72.181,在配置ZStack2的公共网络IP也设置为此IP.

远端子网：远端的一个私有网络段，这里为192.168.0.0/24。

认证密码：两边通信的校验密码。支持所有长度的任意字符串。

创建zstack1的IPsec后，打开ZStack2的新建IPsec页面。如图14-4-6-4所示。
...
在以上的场景中，创建IPsec隧道时提供默认的IKE和ESP配置信息，该配置信息隐藏在**高级设置**中。如果管理员设计ZStack云路由与支持IPsec隧道的第三方设备对接，则需要两端协商具体的配置信息。
