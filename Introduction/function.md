# 1.2 主要功能
ZStack产品是私有云管控系统，提供了对用户数据中心的计算、存储、网络等资源的管理和调度。用户使用ZStack产品可以快速的配置私有云环境，并通过ZStack快速的创建云主机、分配云盘和自动配置云主机网络。
...
| CPU | 支持资源超分配 |
| 内存 | 支持资源超分配 |
| 云盘 | 支持创建共享云盘和普通云盘，云盘云主机挂载、卸载、定时任务、创建快照、恢复快照等操作 |
| 镜像 | 支持添加qcow2、raw、ISO、vmdk等格式，支持在线创建镜像
